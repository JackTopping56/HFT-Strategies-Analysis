...
model = joblib.load('svm_market_model.joblib')

# Prepare the test dataset
numeric_features = df_test.select_dtypes(include=[np.number]).columns.tolist()
features = [col for col in numeric_features if col != 'close']  # Assuming 'close' is the target variable
...
predictions_df.to_csv('market_svm_predictions.csv', index=False)
print("Predictions saved to 'market_svm_predictions.csv'.")
